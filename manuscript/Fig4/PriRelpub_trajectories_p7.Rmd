...
gbm_mer_p7 <-merge(x = Primary_p1_query, y= Relapse_p2_query, project='Patient', add.cell.ids = pati_list$ID,merge.data=TRUE)

gbm_mer_p7  <- NormalizeData(gbm_mer_p7)
...
gbm_mer_p7 <-SCTransform(gbm_mer_p7,vars.to.regress=c('percent.mt','nCount_RNA','nFeature_RNA'))
gbm_mer_p7 <- RunPCA(gbm_mer_p7)
gbm_mer_p7 <-RunHarmony(gbm_mer_p7,reduction='pca',dims.use=1:20,group.by.vars='orig.ident',plot.convergence=T)
#gbm_mer_p5 <- RunPCA(gbm_mer_p5)
gbm_mer_p7 <- FindNeighbors(gbm_mer_p7, dims = 1:30)
gbm_mer_p7 <- FindClusters(gbm_mer_p7)

gbm_mer_p7 <- RunUMAP(gbm_mer_p7, dims=1:30)
...
```{r}
total_cell_p7I <- sum(table(Idents(gbm_mer_p7)))
tot_cell_p7S <- table(gbm_mer_p7$Stage)

DimPlot(gbm_mer_p7, group.by='Stage', raster = F)
DimPlot(gbm_mer_p7, group.by='predicted.celltype', raster = F)
...
gbm_gene1 <- as.data.frame(rownames(GetAssayData(object = gbm_mer_p7, slot = "counts")))
colnames(gbm_gene1) <- c("gene_short_name")
rownames(gbm_gene1) <- gbm_gene1$gene_short_name


cds_p7 <- new_cell_data_set(GetAssayData(object = gbm_mer_p7, slot = "counts"),
cell_metadata = gbm_mer_p7@meta.data,
gene_metadata = gbm_gene1)


## Step 1: Normalize and pre-process the data
cds_p7 <- preprocess_cds(cds_p7, num_dim = 20)

## Step 2: Remove batch effects with cell alignment
cds_p7 <- align_cds(cds_p7, alignment_group = "Stage")

## Step 3: Reduce the dimensions using UMAP
cds_p7 <- reduce_dimension(cds_p7)
...
plot_cells(cds_p7, label_groups_by_cluster=FALSE,  color_cells_by = "Stage") + theme(text= element_text(size = 14), legend.position = "bottom", legend.text = element_text(size = 8))

## Step 4: Cluster the cells
cds_p7 <- cluster_cells(cds_p7, resolution = 1e-4)
plot_cells(cds_p7, color_cells_by = "partition")

## Step 5: Learn a graph
cds_p7 <- learn_graph(cds_p7)
...
plot_cells(cds_p7, color_cells_by = "Stage",label_groups_by_cluster=FALSE, label_leaves=FALSE,label_branch_points=FALSE) + theme(legend.position = "bottom")
...
cds_p7 <- order_cells(cds_p7, root_pr_nodes=get_earliest_principal_node(cds_p7))
...
p7_states_PR <- pbuild_p7$data[c('predicted.id', 'cell_color','Stage')] %>% filter(cell_color != "Inf")%>%arrange(desc(cell_color))
tp_bin <- seq(0, 11, by =2)
#p7_stot <- sum(table(cds_p7_PDT$predicted.id))

for(j in 2:length(tp_bin)){
cell_binT2 <- p7_states_PR %>% filter(cell_color <= tp_bin[j])
p7_stot <- sum(table(cell_binT2$predicted.id))
cell_prop <- round((table(cell_binT2$predicted.id, cell_binT2$Stage)/p7_stot)*100,2)
cell_prop <- as.data.frame(cell_prop)
Pbin_nam <-paste(tp_bin2[j-1],"-",tp_bin2[j])
cell_prop$Part <- Pbin_nam
colnames(cell_prop) <- c("State","Condition","Percentage","PDT")
cellSt_traj_P7 <- rbind(cellSt_traj_P7, cell_prop)
}
colnames(cellSt_traj_P7) <- c("State","Condition","Percentage","PDT")
ggplot(cellSt_traj_P7, aes(fill=State, y=Percentage, x=Condition)) + geom_bar(position="stack", stat="identity") + ylim(0,100)+  theme_classic()+ theme(text = element_text(size = 18)) + xlab("Pseudotime") +  facet_grid(~factor(PDT, levels=c("0 - 2", "2 - 4", "4 - 6", "6 - 8", "8 - 10")))

ggplot(cellSt_traj_P7A, aes(fill=State, y=Percentage, x=Condition)) + geom_bar(position="stack", stat="identity") + ylim(0,100)+  theme_classic()+ theme(text = element_text(size = 18), axis.text.x = element_text(angle = 90)) + xlab("Pseudotime") +  facet_grid(~factor(PDT, levels=c("0 - 2", "2 - 4", "4 - 6", "6 - 8", "8 - 10")))
